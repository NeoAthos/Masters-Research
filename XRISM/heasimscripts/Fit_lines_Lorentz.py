...
class fit_line():
    ...
    def fit(self, wavelength, bound, z, title, amplitudes=None, centers=None,
        ...
        if ncomp == 1:
            ...
            def composite_model(ncomp):
                ...
                for i in range(ncomp - 1):
                    ...
                    model += LorentzianModel(prefix='l%s_'%(i+2))
                return model
        ...
        for i in range(ncomp):
            ...
            params['l{}_amplitude'.format(i+1)].set(value=amplitudes[i], min=0.001)
            params['l{}_center'.format(i+1)].set(value=centers[i])
            params['l{}_sigma'.format(i+1)].set(value=sigmas[i])
        ...
        ax1.plot(x, comps['g1_'], ls='--', label='Gauss Comp')
        ax1.plot(x, comps['l1_'], ls='--', label='Lorentz Comp')
        ...
        if ncomp > 1:
            ...
            for i in range(ncomp):
                ...
                ax1.plot(x, comps[prefix], ls='--', label='Gauss Comp {}'.format(i + 1))
                prefix = 'l{}_'.format(i + 1)
                ax1.plot(x, comps[prefix], ls=':', label='Lorentz Comp {}'.format(i + 1))
    ...
    def calculate_lw(self, T, T_err, output, mass=1.2, res=5.0, ncomp=1):
        ...
        lw_th_eV = ((1/2)*mass_kg*(lw_th*1000)**2)/1.602e-19
        ...
        lw_th_err_eV = ((1/2)*mass_kg*(lw_th_err*1000)**2)/1.602e-19
        ...
        lw_i_eV = ((1/2)*mass_kg*(lw_i*1000)**2)/1.602e-19
        ...
        lw_turb_eV = ((1/2)*mass_kg*(lw_turb*1000)**2)/1.602e-19
        ...
        lw_turb_err_eV = ((1/2)*mass_kg*(lw_turb_err*1000)**2)/1.602e-19
...
main(specfile, spec, 0.02, z, 'Ne X', lines.Ne_X, pdf_pages, mass=20.1797,
...
main(specfile, spec, 0.02, z, 'Mg XII', lines.Mg_XII, pdf_pages, mass=24.305,
...
main(specfile, spec, 0.02, z, 'Si XIV', lines.Si_XIV, pdf_pages, mass=28.0855,
