...
def get_volume(asec_kpc,vol):
    volume = vol*((0.492/asec_kpc)*kpc)**3
    return volume

def get_ne0(DA,z,n0,volume,n_p0,n_n0):
    ne0 = (DA * (1 + z) * 10**7 * np.sqrt(n0 * 4 * np.pi * 1.2 / volume)).cgs.value
...
def get_P(ne0, ne_p0, ne_n0, kt_value0, kt_cgs_p0, kt_cgs_n0):
    P0 = 2 * ne0 * kt_value0
...
def get_Mgas(vol,P0,kt_value0,P_p0,kt_cgs_p0,P_n0,kt_cgs_n0):
    
    mean_mol_m = 0.62 #4/(6*(0.75)+0.24+2)
    mp = (1.67e-27)*1000 #mass of proton in g
    m_solar = 1.989e+33 #Solar mass in cgs
    
    volume = (vol.cgs).value
    M_g0 = P0  * mean_mol_m * mp * volume / (kt_value0)
    print(M_g0)
    
    M_g_p0 = np.sqrt((mean_mol_m * mp * volume * P_p0/kt_value0)**2 + (-P0  * mean_mol_m * mp * volume * kt_cgs_p0/kt_value0**2)**2)
    M_g_n0 = np.sqrt((mean_mol_m * mp * volume * P_n0/kt_value0)**2 + (-P0  * mean_mol_m * mp * volume * kt_cgs_n0/kt_value0**2)**2)
    ...
    return mean_mol_m,mp,m_solar,M_g0,M_g_p0,M_g_n0,Mgas_cum0,dM_g_p0,dM_g_n0
...
def get_tcool(vol,P0,P_p0,P_n0,Lx0,Lx_p0,Lx_n0):
    ...
    t_cool0 = (3 * P0 * volume) / (2 * Lx0)
    t_cool_p0  = np.sqrt((3 * volume * P_p0 / (2*Lx0))**2 + (-3 * P0 * volume * Lx_p0/(2 * Lx0**2))**2)
    t_cool_n0  = np.sqrt((3 * volume * P_n0 / (2*Lx0))**2 + (-3 * P0 * volume * Lx_n0/(2 * Lx0**2))**2)
    
    tcool0 = ((t_cool0).to(u.year)).value
    tcool_p0 = ((t_cool_p0).to(u.year)).value
    tcool_n0 = ((t_cool_n0).to(u.year)).value
    
    return P0,P_p0,P_n0,t_cool0,t_cool_p0,t_cool_n0,tcool0,tcool_p0,tcool_n0
...
def do_calc(path, cluster_name, z, arg):

    cosmo = LCDM(70,0.3)
    z=float(z)
    #fout=open(cluster_name+"/"+cluster_name+"_projected_properties.csv",'w')
    fout=open(path+cluster_name+arg+"_properties.csv",'w')
    nh0,nh_p0,nh_n0,kt0,kt_p0,kt_n0,Z0,Z_p0,Z_n0,n0,n_p0,n_n0,stats0,logflux0,logflux_p0,logflux_n0,chi,dof,rechi,volume = np.genfromtxt(path+"specfits_"+arg+".csv",unpack = True)
    ...
    volume=get_volume(asec_kpc,volume) # kpc^3

    ne0, ne_p0, ne_n0 = get_ne0(DA, z, n0, volume, n_p0, n_n0)

    kt_units0,kt_p_units0,kt_n_units0,kt_cgs0,kt_cgs_p0,kt_cgs_n0,kt_value0=kt_units(kt0,kt_p0,kt_n0)

    P0, P_p0, P_n0 = get_P(ne0, ne_p0, ne_n0, kt_value0, kt_cgs_p0, kt_cgs_n0)

    K0,K_p0,K_n0=get_K(kt0,ne0,ne_p0,kt_p0,ne_n0,kt_n0)

    mean_mol_m,mp,m_solar,M_g0,M_g_p0,M_g_n0,Mgas_cum0,dM_g_p0,dM_g_n0=get_Mgas(volume,P0,kt_value0,P_p0,kt_cgs_p0,P_n0,kt_cgs_n0)

    d_l,flux0,flux_p0,flux_n0,Lx0,Lx_p0,Lx_n0=get_flux_lum(cosmo,z,logflux0,logflux_p0,logflux_n0)

    P0,P_p0,P_n0,t_cool0,t_cool_p0,t_cool_n0,tcool0,tcool_p0,tcool_n0=get_tcool(volume,P0,P_p0,P_n0,Lx0,Lx_p0,Lx_n0)

    fout.write("#volume,kT,kT_p,kT_m,Z,Z_p,Z_m,norm,norm_p,norm_m,log10flux,log10flux_p,log10flux_m,reduced_chisquare,ne,ne_p,ne_m,P,P_p,P_m,K,K_p,K_m,Mgas,Mgas_p,Mgas_m,Lx,Lx_p,Lx_n,tcool,tcool_p,tcool_m\n")
    ...
    if isinstance(tcool0, (list, np.ndarray)):
        for j in range(len(tcool0)):
            fout.write("{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}\n".format(volume[j].cgs.value,kt0[j],kt_p0[j], abs(kt_n0[j]), Z0[j], Z_p0[j], abs(Z_n0[j]),n0[j],n_p0[j],abs(n_n0[j]),logflux0[j],logflux_p0[j],abs(logflux_n0[j]),rechi[j], ne0[j], ne_p0[j], abs(ne_n0[j]),P0[j].value, P_p0[j].value, abs(P_n0[j].value), K0[j],  K_p0[j], abs(K_n0[j]), M_g0[j],M_g_p0[j],  abs(M_g_n0[j]),Lx0[j].value, Lx_p0[j].value, abs(Lx_n0[j].value),tcool0[j], tcool_p0[j], abs(tcool_n0[j])))
    else:
        fout.write("{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}\n".format(volume.cgs.value,kt0,kt_p0, abs(kt_n0), Z0, Z_p0, abs(Z_n0),n0,n_p0,abs(n_n0),logflux0,logflux_p0,abs(logflux_n0),rechi, ne0, ne_p0, abs(ne_n0),P0.value, P_p0.value, abs(P_n0.value), K0,  K_p0, abs(K_n0), M_g0,M_g_p0, abs(M_g_n0),Lx0.value, Lx_p0.value, abs(Lx_n0.value),tcool0, tcool_p0, abs(tcool_n0)))
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Use: python {} {} {} {} {}".format(sys.argv[0], 'path', 'cluster_name', 'z', 'region_name'))
    ...
    do_calc(path,cluster_name,z,arg)
