...
cpd specfits_model5_proj.ps/ps
...
foreach base $bases {
   ...
   # @~/scripts/fit_10.tcl
    ...
    error 1. 1 2 3 6 7 12 ;#Calculates the 1-sigma confidence range or the 68.3% confidence range
    ...
    # Print out results
    set outfile [open "${base}_model5_fit_out_freenH.txt" w]
#set outfile [open "new/${base}_fit_out.txt" w]
    ...
    tclout error 2
    scan $xspec_tclout "%f %f" nH_low nH_upp
    puts $outfile "nH $nH"
    puts $outfile "nH_uerr [expr $nH_upp - $nH]"
    puts $outfile "nH_lerr [expr $nH_low - $nH]"
    ...
    tclout param 7
    scan $xspec_tclout "%f" kT1
    tclout error 7
    scan $xspec_tclout "%f %f" kT1_low kT1_upp
    puts $outfile "KT1 $kT1"
    puts $outfile "KT1_uerr [expr $kT1_upp - $kT1]"
    puts $outfile "KT1_lerr [expr $kT1_low - $kT1]"

    tclout param 12
    scan $xspec_tclout "%f" norm1
    tclout error 12
    scan $xspec_tclout "%f %f" norm1_low norm1_upp
    puts $outfile "norm1 $norm1"
    puts $outfile "norm1_uerr [expr $norm1_upp - $norm1]"
    puts $outfile "norm1_lerr [expr $norm1_low - $norm1]"
