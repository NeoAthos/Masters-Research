...
cpd specfits_deproj_mkc_free.ps/ps
...
foreach base $bases {
    if {$sum==0} {
     #    set speclist [glob "${base}_*_grp100.pi"]
        ...
        set spec_sum [lsearch $speclist "${base}_sum_deproj.pi"]
        set speclist [lreplace $speclist $spec_sum $spec_sum]
        set k 1
    	foreach x $speclist {
    	    data 1:${k} ${x}
...
#ignore **:**-0.5,7.0 ;
   ...
   # @~/scripts/fit_10.tcl
    ...
    error 1. 2 3 4 7 8 9 11 ;#Calculates the 1-sigma confidence range or the 68.3% confidence range
    ...
    # Print out results
    set outfile [open "${base}_mkc_free_deproj_fit_out.txt" w]
