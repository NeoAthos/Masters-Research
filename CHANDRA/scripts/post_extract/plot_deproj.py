...
def get_ne0(DA,z,n0,rin,rout,n_p0,n_n0,DV):
    
    ne0 = (DA * (1 + z) * 10**7 * np.sqrt(n0 * 4 * np.pi * 1.2 / DV )).cgs.value
...
def get_Mgas(rin,rout,dr,P0,kt_value0,P_p0,kt_cgs_p0,P_n0,kt_cgs_n0,DV):
    ...
    M_g0 = P0  * mean_mol_m * mp * DV / (kt_value0)
    
    M_g_p0 = np.sqrt((mean_mol_m * mp * DV * P_p0/kt_value0)**2 + (-P0  * mean_mol_m * mp * DV * kt_cgs_p0/kt_value0**2)**2)
    M_g_n0 = np.sqrt((mean_mol_m * mp * DV * P_n0/kt_value0)**2 + (-P0  * mean_mol_m * mp * DV * kt_cgs_n0/kt_value0**2)**2)
...
def get_tcool(r_in,r_out,P0,P_p0,P_n0,Lx0,Lx_p0,Lx_n0, DV):
    ...
    t_cool0 = (3 * P0 * DV.to(u.cm**3)) / (2 * Lx0)
    t_cool_p0  = np.sqrt((3 * DV * P_p0 / (2*Lx0))**2 + (-3 * P0 * DV * Lx_p0/(2 * Lx0**2))**2)
    t_cool_n0  = np.sqrt((3 * DV * P_n0 / (2*Lx0))**2 + (-3 * P0 * DV * Lx_n0/(2 * Lx0**2))**2)
...
fout=open(path+cluster_name+"_deprojected_properties.csv",'w')
pdf0=many_pdf.PdfPages(path+cluster_name+'_plot_deprojected_properties.pdf')
...
rin,rout,nh0,nh_p0,nh_n0,kt0,kt_p0,kt_n0,Z0,Z_p0,Z_n0,n0,n_p0,n_n0,stats0,logflux0,logflux_p0,logflux_n0,chi,dof,rechi = np.genfromtxt(path+"specfits_deproj.csv",unpack = True)
...
DV = ((1/asec_kpc)*kpc)**3 # Because Deproj spectra are normalized to unit
                                     # volume in the same units used in the XFLT
                                     # keywords which is arcsec (because I multiplied
                                     # pixel values by 0.492 and Not by 0.0082 in the
                                     # project_keywords script. If you use 0.0082 in the 
                                     # project_keywords script, change (1/asec_kpc) to
                                     # (60/asec_kpc) here.

rin,rout,r,dr=get_r(DA,asec_kpc,rin,rout)

ne0,ne_p0,ne_n0=get_ne0(DA,z,n0,rin,rout,n_p0,n_n0,DV)
...
mean_mol_m,mp,m_solar,r_out,r_in,r_avg,dr_kpc,dr_cgs,M_g0,M_g_p0,M_g_n0,Mgas_cum0,dM_g_p0,dM_g_n0=get_Mgas(rin,rout,dr,P0,kt_value0,P_p0,kt_cgs_p0,P_n0,kt_cgs_n0,DV)

d_l,flux0,flux_p0,flux_n0,Lx0,Lx_p0,Lx_n0=get_flux_lum(cosmo,z,logflux0,logflux_p0,logflux_n0)

r_out,r_in,P0,P_p0,P_n0,t_cool0,t_cool_p0,t_cool_n0,tcool0,tcool_p0,tcool_n0=get_tcool(r_in,r_out,P0,P_p0,P_n0,Lx0,Lx_p0,Lx_n0,DV)
