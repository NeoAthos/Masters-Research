...
if len(sys.argv) != 4:
    sys.stderr.write(
        "Usage %s infile_evt2.fits backfile_evt2.fits ccd_id" % sys.argv[0])
    sys.exit(1)
...
#        ccd = '7'
    ...
    cmd = 'dmstat "%s[ccd_id=%s][energy=9500:12000][cols pi]"' % (
        filename, ccd)
    good = None
...
ehead.add_history(
    'Exposure time decreased by %f to match 9.5-12 keV rate' % ratio)
